...
## Class MyNode
class MyNode:
  def __init__(self, id = VACANT_INDEX, label = VACANT_LABEL, to_nodes = set()):
    self.id = id
    self.to_nodes = to_nodes.copy()   ## list id_nodes that neighbor
    self.label = label 
...
class MyGraph:
  ...
  def loadEdge (self, filename):
    with open(filename, 'r') as f:
      for line in f.readlines():
        ...
        self.edges.add(MyEdge((line[1], line[0], VACANT_LABEL)))
        self.nodes[line[0]].to_nodes.add(line[1])
        self.nodes[line[1]].to_nodes.add(line[0])
  ...
  def addEdge(self, edge):
    self.edges.add(edge)
    self.edges.add(MyEdge((edge[1], edge[0], VACANT_LABEL)))
    self.nodes[edge[0]].to_nodes.add(edge[1])
    self.nodes[edge[1]].to_nodes.add(edge[0])
  ...
  def rightMostPatExt(self, dfscode):   
    ...
    if rmNode < 0:   ## dfs is None
      ...
      temp = min(i[2] for i in res)
      return res, MyDFSEdge((0, 1, temp))
    else:
      minExt = [rmNode, -1]
        ...
        for u in R:      ## O(|R|) ~ O(|V(c)|)
          for x in self.nodes[i[u]].to_nodes: ## O(|V(g)|)
            if x not in i:        ## O(|V(c)|) 
              ...
              if minExt[1] < u:   ## find max
                minExt[1] = u
        
    if minExt[0] != rmNode:
        temp = MyDFSEdge((rmNode, minExt[0], (VACANT_INDEX, VACANT_LABEL, VACANT_INDEX)))
    else:
        temp = MyDFSEdge((minExt[1], rmNode+1,(VACANT_INDEX, VACANT_LABEL, VACANT_INDEX)))
  ...
  def MinerPattern2(self, s):
    ...
    for n in self.nodes:
      for nn in n.to_nodes:
        if n.label <= self.nodes[nn].label:
          pattern = (n.label, VACANT_LABEL, self.nodes[nn].label)
          e1, e2 = n.id, nn
        else:
          pattern = (self.nodes[nn].label, VACANT_LABEL, n.label)
          e1, e2 = nn, n.id
        if pattern not in QMF:
          QMF[pattern] = [[e1, e2]]
        else:
          QMF[pattern].append([e1, e2])
    for key in QMF:
  ...
  def FPMiner(self, s):
    print('Before - NumOfNodes: ', len(self.nodes), 'NumOfEdges: ', len(self.edges))
    g1 = self.drop_vertex_under_support(s)
    QSet, s2, MF2, nodes = g1.MinerPattern2(s)
    g2 = g1.pruneEdge(MF2, nodes)
    print('After - NumOfNodes: ', len(self.nodes), 'NumOfEdges: ', len(self.edges))
    res = MyCodeGraph()
    for i in QSet:
...
class MyDFSEdge(tuple):
  ...
  def eqOrder(self, other):
    return (self[0] == other[0] and self[1] == other[1]) or (self[1] == other[0] and self[0] == other[1])
